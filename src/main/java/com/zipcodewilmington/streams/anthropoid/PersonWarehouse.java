...
public final class PersonWarehouse implements Iterable<Person> {
    ...
    public List<String> getNames() {
       return people.stream().map(element -> element.getName())
    ...
    public Stream<Person> getUniquelyNamedPeople() {
        Map<String, Person> uniqueNames =  people.stream()
    ...
    public Stream<Person> getUniquelyNamedPeopleStartingWith(Character character) {
        return getUniquelyNamedPeople()
                .filter(people -> people.getName().indexOf(character) == 0);
    ...
    public Stream<Person> getFirstNUniquelyNamedPeople(int n) {
        Stream<Stream<Person>> streamstream = people.stream()
                .map(i -> getUniquelyNamedPeople());

        return streamstream.flatMap(stream -> stream).limit(n);
